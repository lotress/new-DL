...
option['loss'] = lambda opt, model, y, out, *_: F.cross_entropy(out, y[:,0], reduction='none')
option['criterion'] = lambda y, out, mask, *_: (out[:,1:vocab].max(-1)[1] + 1).ne(y[:,0]).float() * mask.float()
try:
